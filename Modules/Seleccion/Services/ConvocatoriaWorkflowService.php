...
class ConvocatoriaWorkflowService
{
    public function __construct(
        private readonly ConvocatoriaModel $model = new ConvocatoriaModel()
    ) {}
    ...
    {
        return $this->model->select('selec_convocatorias.*, t.tco_nombre, e.eco_nombre, e.eco_codigo')
    ...
    public function guardar(array $data, ?int $id = null): array
    {
        $rules = [
            ...
            'con_tco_ide' => 'required|is_not_unique[selec_tipos_convocatoria.tco_ide]',
            'con_eco_ide' => 'required|is_not_unique[selec_estados_convocatoria.eco_ide]',
            'con_anio'    => 'required|integer'
        ];
    ...
    /**
     * Obtiene los datos detallados según la subsección enviada
     */
    public function obtenerDetalleSeccion(int $id, string $seccion): array
    {
        return match ($seccion) {
            'cargos' => [
                'cargos' => (new ConvocatoriaCargoModel())->where('cco_con_ide', $id)->findAll()
            ],
            'cronograma' => [
                'cronograma' => (new ConvocatoriaEtapaModel())->where('cet_con_ide', $id)->orderBy('cet_orden', 'ASC')->findAll()
            ],
            'documentos' => [
                'documentos' => (new ConvocatoriaDocumentoModel())->where('cod_con_ide', $id)->findAll()
            ],
            'requisitos', 'anexos', 'resumen' => [
                'convocatoria' => $this->obtener($id)
            ],
            default => []
        };
    }
