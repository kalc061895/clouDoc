...
<div class="card bg-white p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0 small">Administración de profesiones</p>
        </div>
...
<div class="modal fade" id="modalprofesiones" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
        <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold" id="modalTitulo">Nuevo Profesión</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
        </div>
        <form id="formprofesiones">
            <div class="modal-body py-3">
                <input type="hidden" id="pro_ide">
                <div class="row g-3">
                        <div class="col-md-6"><label class="form-label small fw-bold">Código</label><input type="text" class="form-control" id="pro_codigo"></div>
                        <div class="col-md-6"><label class="form-label small fw-bold">Nombre <span class="text-danger">*</span></label><input type="text" class="form-control" id="pro_nombre" required></div>
                        <div class="col-md-6"><label class="form-label small fw-bold">Estado <span class="text-danger">*</span></label><select class="form-select" id="pro_estado" required><option value="ACTIVO">Activo</option><option value="INACTIVO">Inactivo</option></select></div>
                </div>
            </div>
...
<script>
    const urlBaseApi = '<?= base_url('seleccion/api/profesiones') ?>';
    const tableId = '#tablaprofesiones';
    const formId = '#formprofesiones';
    const modal = new bootstrap.Modal(document.getElementById('modalprofesiones'));
    let tabla;
    ...
    $(document).ready(function () {
        tabla = $(tableId).DataTable({
            ajax: { url: urlBaseApi, type: 'GET', dataSrc: function (response) {
                ...
                registros.clear();
                (response.data || []).forEach(row => registros.set(String(row.pro_ide), row));
                return response.data || [];
            }},
            columns: [
                { data: 'pro_codigo', render: function(value) { return escapeHtml(value ?? ''); } },
                { data: 'pro_nombre', render: function(value) { return escapeHtml(value ?? ''); } },
                { data: 'pro_estado', render: function(value) { return escapeHtml(value ?? ''); } },
                { data: null, orderable: false, className: 'text-end', render: function (_, __, row) {
                    return '<button class="btn btn-outline-warning btn-sm me-1 rounded-circle p-1 editar" data-id="' + row.pro_ide + '" title="Editar" style="width:32px;height:32px"><iconify-icon icon="lucide:edit"></iconify-icon></button>' +
                        '<button class="btn btn-outline-danger btn-sm rounded-circle p-1 eliminar" data-id="' + row.pro_ide + '" title="Eliminar" style="width:32px;height:32px"><iconify-icon icon="lucide:trash-2"></iconify-icon></button>';
                }}
            ],
    ...
    function abrirModalCrear() {
        ...
        $('#modalTitulo').text('Nuevo Profesión');
        modal.show();
    }
    function abrirModalEditar(data) {
        ...
        $('#pro_estado').val(data.pro_estado ?? 'ACTIVO');
