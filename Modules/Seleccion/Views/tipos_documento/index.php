...
<div class="card bg-white p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0 small">Administración de tipos de documento</p>
        </div>
    ...
    <div class="table-responsive">
        <table id="tablatiposDocumento" class="table table-hover align-middle w-100">
            <thead><tr>
                    <th>Código</th>
                    <th>Nombre</th>
                    <th>Estado</th>
...
<div class="modal fade" id="modaltiposDocumento" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
        <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold" id="modalTitulo">Nuevo Tipo de Documento</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
        </div>
        <form id="formtiposDocumento">
            <div class="modal-body py-3">
                <input type="hidden" id="tdo_ide">
                <div class="row g-3">
                        <div class="col-md-6"><label class="form-label small fw-bold">Código <span class="text-danger">*</span></label><input type="text" class="form-control" id="tdo_codigo" required></div>
                        <div class="col-md-6"><label class="form-label small fw-bold">Nombre <span class="text-danger">*</span></label><input type="text" class="form-control" id="tdo_nombre" required></div>
                        <div class="col-md-6"><label class="form-label small fw-bold">Estado <span class="text-danger">*</span></label><select class="form-select" id="tdo_estado" required><option value="ACTIVO">Activo</option><option value="INACTIVO">Inactivo</option></select></div>
                </div>
            </div>
...
<script>
    const urlBaseApi = '<?= base_url('seleccion/api/tipos-documento') ?>';
    const tableId = '#tablatiposDocumento';
    const formId = '#formtiposDocumento';
    ...
    $(document).ready(function () {
        tabla = $(tableId).DataTable({
            ajax: { url: urlBaseApi, type: 'GET', dataSrc: function (response) {
                ...
                registros.clear();
                (response.data || []).forEach(row => registros.set(String(row.tdo_ide), row));
                return response.data || [];
            }},
            columns: [
                { data: 'tdo_codigo', render: function(value) { return escapeHtml(value ?? ''); } },
                { data: 'tdo_nombre', render: function(value) { return escapeHtml(value ?? ''); } },
                { data: 'tdo_estado', render: function(value) { return escapeHtml(value ?? ''); } },
                { data: null, orderable: false, className: 'text-end', render: function (_, __, row) {
                    return '<button class="btn btn-outline-warning btn-sm me-1 rounded-circle p-1 editar" data-id="' + row.tdo_ide + '" title="Editar" style="width:32px;height:32px"><iconify-icon icon="lucide:edit"></iconify-icon></button>' +
                        '<button class="btn btn-outline-danger btn-sm rounded-circle p-1 eliminar" data-id="' + row.tdo_ide + '" title="Eliminar" style="width:32px;height:32px"><iconify-icon icon="lucide:trash-2"></iconify-icon></button>';
                }}
            ],
    ...
    function abrirModalEditar(data) {
        if (!data) return;
        $(formId)[0].reset();
        $('#tdo_ide').val(data.tdo_ide);
        $('#tdo_codigo').val(data.tdo_codigo ?? '');
        $('#tdo_nombre').val(data.tdo_nombre ?? '');
        $('#tdo_estado').val(data.tdo_estado ?? 'ACTIVO');
        $('#modalTitulo').text('Editar Tipo de Documento');
        modal.show();
    }
    function guardar() {
        const id = $('#tdo_ide').val();
        const editar = id !== '';
        const datos = {
            tdo_codigo: $('#tdo_codigo').val(),
            tdo_nombre: $('#tdo_nombre').val(),
            tdo_estado: $('#tdo_estado').val()
        };
