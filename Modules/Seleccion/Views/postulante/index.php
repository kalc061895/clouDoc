...
<div class="card bg-white p-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 text-dark fw-bold d-flex align-items-center">
                <iconify-icon icon="solar:user-speak-bold" class="me-2 text-primary"
    ...
    <ul class="nav nav-pills mb-4 gap-2" id="postulacionTabs" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active rounded-pill px-4 d-flex align-items-center fw-semibold"
                id="mis-postulaciones-tab" data-bs-toggle="pill" data-bs-target="#mis-postulaciones" type="button"
                role="tab">
        ...
        <li class="nav-item" role="presentation">
            <button class="nav-link rounded-pill px-4 d-flex align-items-center fw-semibold" id="convocatorias-tab"
                data-bs-toggle="pill" data-bs-target="#convocatorias" type="button" role="tab">
    ...
    <div class="tab-content" id="postulacionTabsContent">
            ...
            <?php if (empty($misPostulaciones)): ?>
                <div class="card border-0 shadow-sm p-5 text-center bg-white rounded-3">
                    <div class="text-muted mb-3">
                        <iconify-icon icon="solar:folder-open-bold-duotone" class="text-secondary"
                            style="font-size: 4rem;"></iconify-icon>
                    </div>
                    <h5 class="fw-bold text-dark">Aún no registras postulaciones</h5>
                    <p class="text-muted small mb-3">Ingresa a la pestaña <strong>"Convocatorias Disponibles"</strong> para
                        presentarte a un concurso público.</p>
                    <div>
                        <button class="btn btn-primary btn-sm rounded-pill px-4" onclick="$('#convocatorias-tab').click()">
                            <iconify-icon icon="solar:magnifier-bold" class="me-1"></iconify-icon> Ver Convocatorias
                            Abiertas
                        </button>
                    </div>
                </div>
                                ...
                                <div class="card-body d-flex flex-column p-4">
                                    <div class="d-flex justify-content-between align-items-start mb-3">
                                        ...
                                        <?php if ((int) ($post['pto_confirmado'] ?? 0) === 1): ?>
                                            <span
                                                class="badge bg-success-subtle text-success border border-success-subtle fw-bold">PRESENTADO</span>
                                        <?php else: ?>
                                            <span
                                                class="badge bg-warning-subtle text-warning border border-warning-subtle fw-bold">EN
                                                BORRADOR</span>
                                        <?php endif; ?>
                                    </div>
                                    ...
                                    <div class="small text-muted mb-3 mt-auto">
                                        <div class="d-flex align-items-center mb-1">
                                            <iconify-icon icon="solar:hashtag-square-bold"
                                                class="me-1 text-primary"></iconify-icon>
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            Fecha:
                                            <?= !empty($post['pto_fecha_presentacion']) ? date('d/m/Y h:i A', strtotime($post['pto_fecha_presentacion'])) : date('d/m/Y h:i A', strtotime($post['created_at'])) ?>
                                        </div>
                                    </div>

                                    <div class="d-grid pt-2 border-top">
                                        <a href="<?= base_url('seleccion/postulacion/ver/' . $post['pto_ide']) ?>"
        ...
        <div class="tab-pane fade" id="convocatorias" role="tabpanel">
            <?php if (empty($convocatoriasAbiertas)): ?>
                <div class="card border-0 shadow-sm p-5 text-center bg-white rounded-3">
                    <div class="text-muted mb-3">
                        <iconify-icon icon="solar:bell-off-bold-duotone" class="text-secondary"
                            style="font-size: 4rem;"></iconify-icon>
                    </div>
                        ...
                        <div class="col-md-6 col-lg-4">
                            <div class="card border-0 shadow-sm h-100 rounded-3 bg-white border-top border-4 border-success">
                                <div class="card-body d-flex flex-column p-4">
                                    <div class="mb-2">
                                        <span
                                            class="badge bg-success-subtle text-success border border-success-subtle fw-bold">POSTULACIÓN
                                            ABIERTA</span>
                                    </div>

                                    <h5 class="fw-bold text-dark mb-1"><?= esc($conv['con_nombre']) ?></h5>
                                    <span class="text-muted small mb-3">N°
                                        <?= esc($conv['con_numero'] ?? $conv['con_codigo']) ?></span>
                                    ...
                                    <button
                                        onclick="confirmarInicioPostulacion(<?= $conv['con_ide'] ?>, '<?= esc($conv['con_nombre'], 'js') ?>')"
                                        class="btn btn-primary btn-sm rounded-pill d-flex align-items-center justify-content-center">
                                        <iconify-icon icon="solar:pen-new-square-bold" class="me-1"
                                            style="font-size: 1.1rem;"></iconify-icon>
